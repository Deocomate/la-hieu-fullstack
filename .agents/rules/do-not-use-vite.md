...
5. **Cấu hình Tailwind:** Nếu cần cấu hình custom theme (extend colors, fonts...), hãy viết trực tiếp vào `<script> tailwind.config = { ... } </script>` ngay trong file layout/blade (ví dụ: `resources/views/components/clients/styles.blade.php`).
